...
3. The planner passcode is `mavion` (case-insensitive).
...
Planner data uses the Redis key `mavion:planner:alexander:v1`. Sessions expire after seven days. Revision checks prevent an older browser tab from silently overwriting newer work. If Redis is not connected, the planner automatically runs in clearly labeled demo mode; demo changes last for the open page and reset on refresh.
