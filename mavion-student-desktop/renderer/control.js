(() => {
  ...
  let toastTimer, countdownTimer, seconds = 30;
  ...
  const showInactivity = () => { clearInactivity(); idleToast.hidden = false; countdownTimer = window.setInterval(() => { seconds -= 1; countdown.textContent = seconds; if (seconds <= 0) activate(); }, 1000); };
  ...
  document.getElementById('idleStay').addEventListener('click', clearInactivity);
  ...
  window.mavion.getIdleLock().then(enabled => idle.checked = enabled);
